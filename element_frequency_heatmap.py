...
from matplotlib.colors import LogNorm, Normalize  # 已经有 LogNorm，这里只是提醒
...
def main() -> None:
    ...
    if args.secondary_csv:
        ...
        # 决定 secondary 用哪一列作为 formula / elements
        secondary_formula = (
        ...
        # 各自单独画一张图，各自用自己的 scale（不传 shared_norm）
        primary_out = Path("figs/primary_element_frequency.pdf")
        secondary_out = Path("figs/secondary_element_frequency.pdf")
        primary_out.parent.mkdir(parents=True, exist_ok=True)

        # 1) Computational / primary
        plot_heatmap(
            primary_heat_df,
            primary_label_df,
        ...
        print(f"\nPrimary heatmap saved to {primary_out}")

        # 2) Experimental / secondary
        plot_heatmap(
            secondary_heat_df,
            secondary_label_df,
            output_path=secondary_out,
            color_la_ac=args.color_la_ac,
            title=args.secondary_label,
            show_colorbar=True,
        )
        print(f"Secondary heatmap saved to {secondary_out}")
